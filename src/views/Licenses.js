...
import {Upload, Col, Collapse, Row, Tabs, Result, Input, Divider, Tooltip, Form, Icon} from "antd";
import Table from "../components/Table";
import Button, {ConfirmButton} from "../components/Button";
import licenceStore from "../store/LicenceStore";
import Select from "../components/Select";
import {Link} from "react-router-dom";
import LicenseListTable from "../components/LicenseListTable";
...
export default class Licenses extends React.Component {
  componentDidMount() {
    adminStore.createLog({
      action: 'Перешел на страницу мои лицензии'
    });
  }
  ...
  render() {
    ...
    return <>
      <Button><Link to="/license">Подать заявку</Link></Button>
      ...
      <Tabs destroyInactiveTabPane>
        <Tabs.TabPane key={'1'} tab="Действующие">
          {/*<Table style={{marginTop: 15}}
                 columns={columns}
                 data={dataSource}/>*/}
          <LicencesList userId={appStore.user?.id} statusId="20" isOwner/>
        </Tabs.TabPane>
        ...
        <Tabs.TabPane key={'2'} tab="Проекты">
          {/*<Table style={{marginTop: 15, width: 500}}
                 ...
                 data={dataSource}
          />*/}
          <LicencesList userId={appStore.user?.id} statusId="1" isOwner/>
        </Tabs.TabPane>
        <Tabs.TabPane key={'3'} tab="На рассмотрении">
          {/*<Table style={{marginTop: 15, width: 600}}
                 ...
                 data={dataSource}
          />*/}
          <LicencesList userId={appStore.user?.id} statusId="10" isOwner/>
        </Tabs.TabPane>
        ...
        <Tabs.TabPane key={'4'} tab="Отклоненные">
          {/*<Table style={{marginTop: 15, width: 800}}
                 ...
                 data={dataSource}
          />*/}
          <LicencesList userId={appStore.user?.id} statusId="30" isOwner/>
        </Tabs.TabPane>
        ...
        <Tabs.TabPane key={'5'} tab="Прекращенные">
          {/*<Table style={{marginTop: 15, width: 800}}
                 ...
                 data={dataSource}
          />*/}
          <LicencesList userId={appStore.user?.id} statusId="40" isOwner/>
        </Tabs.TabPane>
